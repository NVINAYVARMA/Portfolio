...
export default function PortraitMorph({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL not supported for PortraitMorph, falling back to static image", err);
      return;
    }

    const gl = renderer.gl;
    const canvas = gl.canvas;
    canvas.className = "portrait-morph-canvas";
    ...
    const tick = () => {
      ...
      const stiffness = hoverRef.current ? 3.5 : 2.5;
      ...
      program.uniforms.uFocusY.value = focusY;

      renderer.render({ scene });
      raf = requestAnimationFrame(tick);
    };

    const targetB = srcB || srcA;
    Promise.all([loadImage(srcA, texA), loadImage(targetB, texB)])
      .then(() => {
        setReady(true);
      ...
      .catch((err) => {
        console.warn("Portrait images failed to load into WebGL textures", err);
        setReady(false);
      });
    ...
    const onPointerLeave = (e) => {
      ...
      const y = 1 - (e.clientY - rect.top) / rect.height;
      originRef.current = [x, y];
      directionRef.current = computeEdgeDirection(x, y).map((v) => -v);
  ...
  return (
    <div
      ...
      role="img"
      aria-label={alt}
      className={`portrait-morph-wrap ${className}`}
      style={{ aspectRatio }}
    >
      {!ready && (
        <img
          ...
          alt={alt}
          draggable={false}
          className="portrait-morph-fallback"
        />
      )}
    </div>
  );
